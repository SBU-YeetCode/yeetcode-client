	...
	React.useEffect(() => {
		...
		if (
			...
			const timeDifference =
				new Date().getTime() - new Date(questionProgress.dateStarted).getTime()
			if (timeDifference >= question.timeLimit) {
				...
				switch (question.gameType) {
					...
					case Gametype.Spotthebug:
						if (
							tempAnswer.spotTheBug == null ||
							isNaN(parseInt(tempAnswer.spotTheBug))
						)
				...
				{questionProgress.dateStarted && (
					<Flex
						...
						<Heading as='h2' fontSize='2xl'>
							{`${formatDistanceStrict(
								questionProgress.completed
									? new Date(questionProgress.dateCompleted)
									: time,
					...
					<Wrap>
						{new Array(question.lives).fill(0).map((_, i) => (
							<Icon
								...
								boxSize={8}
								as={
									i < questionProgress.livesLeft ? AiFillHeart : AiOutlineHeart
								}
								color='red.500'
							/>
						))}
